...
export default function LinkParent({ link }) {
  ...
  const dropClasses = show
    ? "transition ease-out duration-500 opacity-100 translate-y-0  z-10"
    : "transition ease-in duration-150 opacity-0 translate-y-2  -z-1 pointer-events-none";
    ...
    <>
      ...
      >
        <div className="rounded-sm shadow-md ring-1 ring-black ring-opacity-5 overflow-hidden">
          <div className="bdf relative grid gap-6 bg-whiteAlpha-600 px-5 py-6 sm:gap-8 sm:p-8">
            {link.child.map((item) => (
              <Link key={item.label} href="#">
                <a className="-m-3 p-3 flex items-start rounded-sm hover:bg-whiteAlpha-400">
                  <Photograph />
                  <div className="ml-4">
